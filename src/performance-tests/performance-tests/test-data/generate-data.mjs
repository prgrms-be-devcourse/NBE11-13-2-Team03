...
const secretBytes = createHash("sha512")
  ...
  .digest();
const jwtSecretKeyBase64 = secretBytes.toString("base64");
...
function createAccessToken(userId) {
  ...
  const signature = createHmac("sha512", secretBytes)
...
const users = Array.from({ length: USER_COUNT }, (_, index) => {
  ...
  return {
    ...
    nickname: `perf-user-${String(id).padStart(4, "0")}`,
    email: `perf-user-${String(id).padStart(4, "0")}@example.test`,
    role: "USER",
    provider: "KAKAO",
...
const goods = Array.from({ length: 104 }, (_, index) => {
  const id = index + 1;
  return {
    id,
    name: `performance-goods-${String(id).padStart(3, "0")}`,
    description: `k6 concurrency fixture ${id}`,
    price: 10_000 + id,
    image_url: null,
    status: "ACTIVE",
    created_at: CREATED_AT,
...
const sales = [
  sale(1, 100, "oversell-hotspot"),
  sale(2, 1_000, "single-row-lock-capacity"),
  ...Array.from({ length: DISTRIBUTED_SALE_COUNT }, (_, index) =>
    sale(index + 3, 10, "distributed-baseline")
  ),
  sale(103, 100, "duplicate-purchase-race"),
  sale(104, 100, "cancel-race")
];

// One stock unit is already reserved by the seeded pending purchase.
sales.find(({ id }) => id === 104).remaining_stock = 99;
...
const output = {
  metadata: {
    ...
    warning: "The bundled JWT secret and tokens are test-only. Never use them in production.",
    requiredEnvironment: {
      JWT_SECRET_KEY: jwtSecretKeyBase64,
      JWT_ISSUER: ISSUER
    },
    activePeriod: {
    ...
    importOrder: ["users", "goods", "sales", "purchases", "payments"],
  ...
  volumePlan: {
    users: {
      count: USER_COUNT,
      reason: "1,000 unique VUs plus one duplicate-purchase actor and one cancellation actor"
    },
    goods: {
      count: 104,
      reason: "one goods row per sale fixture"
    },
    sales: {
      count: 104,
      reason: "two hotspot fixtures, 100 distributed fixtures, and two race-condition fixtures"
    },
    purchases: {
      count: 1,
      reason: "pending purchase used by the concurrent cancellation test"
    },
    payments: {
      count: 1,
      reason: "READY payment paired with the pending cancellation purchase"
    },
    accessTokens: {
      count: USER_COUNT,
      reason: "authenticated k6 requests without invoking Kakao OAuth during the measured run"
    }
  },
  scenarios: {
    oversellHotspot: {
      description: "1,000 unique users compete for 100 units on one locked sale row.",
      saleIds: [1],
      ...
      expectedInvariant: {
        ...
        allowedRejectCodes: ["SALE_002", "SALE_004"],
        finalRemainingStock: 0,
        finalSaleStatus: "SOLD_OUT"
      }
    },
    singleRowLockCapacity: {
      description: "1,000 unique users compete on one sale with sufficient stock to expose lock queue latency.",
      saleIds: [2],
      actorUserIds: { from: 1, to: 1_000 },
      requestsPerActor: 1,
      expectedInvariant: {
    ...
    duplicatePurchaseRace: {
      description: "One authenticated user fires 50 concurrent requests at the same sale.",
      saleIds: [103],
      actorUserIds: [1_001],
  ...
  payments: [
    {
      id: 1,
      purchase_id: 1,
      order_id: "GUDIT_PERF_CANCEL_RACE_0001",
      payment_key: null,
      amount: 10_104,
      status: "READY",
      approved_at: null,
...
const outputPath = resolve(here, "generated", "performance-test-data.json");
mkdirSync(dirname(outputPath), { recursive: true });
writeFileSync(outputPath, `${JSON.stringify(output, null, 2)}\n`, "utf8");
console.log(`Created ${outputPath}`);
console.log(JSON.stringify(output.volumePlan, null, 2));
